    ...
    return (
        <nav className="flex fixed top-0 w-screen justify-between p-5 border-b-2">
            <div className="flex gap-2">
                ...
                <ul className={`${showMenu?'left-0': '-left-52'} transition-all duration-1000 md:left-auto fixed px-5 pt-2 md:pt-0 sub-nav border-2 md:border-none pb-36 md:pb-0 h-screen md:h-auto top-16 mt-1 md:mt-0 left-0 md:static md:justify-start md:items-center md:flex md:gap-5 md:pl-14`}>
                ...
                <li><span className="flex items-center gap-1"><Link to='/category'>Category</Link><button onClick={toggleChev}>{showSubMenu?<FaChevronDown/>:<FaChevronRight/>}</button>
                    ...
                    <ul className={`pl-5 transition duration-700 ${showSubMenu?'md:fixed md:opacity-100 md:h-auto md:grid grid-auto-flow md:w-screen md:top-20 md:left-0 md:gap-3': ' md:opacity-0 md:h-0 md:w-0'}`}>
